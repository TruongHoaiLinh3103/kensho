...
const Login = () => {
    ...
    const loggoutScheme = yup.object().shape({
        email: yup.string().email().required(),
        password: yup.string().max(30).min(4).required(),
        cfpassword: yup.string().min(4).max(30).required()
    });
    const nextSignUp = () => {
        setLogin(false)
    }
    ...
    const loggin = async (e) => {
        ...
        const isValid = await loginScheme.isValid(formData);
        if(isValid === false){
    ...
    const loggout = async (e) => {
        ...
        let formData = {
            email: e.target[0].value,
            password: e.target[1].value,
            cfPassword: e.target[2].value
        }
        const isValid = await loggoutScheme.isValid(formData);
        if(isValid === false){
            alert("Invalid. Please try again.")
        }else{
            alert("SUCCESS!")
    ...
    return (
        <div className="Login">
            <form className="form" data-aos="fade-up" onSubmit={login === true ? (e) => loggin(e) : (e) => loggout(e)}>
                ...
                <>
                    ...
                    <div className="inputForm">
                        ...
                        <input placeholder="Enter your Comfirm Password" className="input" type="password"/>
                    </div>
                </>
                }
                <div className="flex-row">
                    <div className='flex-row_remember' style={{
                        display: login === false ? "none" : ""
                    }}>
                    ...
                    {login === true ?
                        ...
                        :
                        <span className="span">Change password!</span>
                    }
                </div>
                {login === true ?
                    <>
                        {/* <button className="button-submit" onClick={() => toBeChecked()}>Sign In</button> */}
                        <input type='submit' value="Sign in"/>
                        <p className="p">Don't have an account? <span className="span" onClick={() => nextSignUp()}>Sign Up</span></p>
                    </>
                    :
                    <>
                        <input type='submit' value="Sign up"/>
                        <p className="p">I have an account! <span className="span" onClick={() => nextSignIn()}>Sign In</span></p>
                    </>
                }
